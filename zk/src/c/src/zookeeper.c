...
**
**  To work around Ruby's threading peculiarities, we wrap completions
**  in a ruby thread.
*/
...
static void ruby_data_completion_wrapper_2(struct data_completion_data *cb)
  {
  LOG_DEBUG(("--- invoking ruby callback. current data:\n\
  -----------------------\n\
  cb->dc         0x%08x\n\
  cb->rc         %d\n\
  cb->value      0x%08x\n\
...
static void ruby_data_completion_wrapper(data_completion_t dc, int rc, char *value, int value_len, struct Stat *stat, void *ctx)
  {
  ...
  rb_thread_create((void *)ruby_data_completion_wrapper_2, cb);
  }
...
if ( cb->stat )
  LOG_DEBUG(("\
        ...
        ->pzxid        %ld\n\
  =======================",
    cb->stat->czxid, cb->stat->mzxid, cb->stat->ctime, cb->stat->mtime,
    cb->stat->version, cb->stat->cversion, cb->stat->aversion,
    cb->stat->ephemeralOwner, cb->stat->dataLength, cb->stat->numChildren,
    cb->stat->pzxid ));
...
static void ruby_stat_completion_wrapper(stat_completion_t dc, int rc, struct Stat *stat, void *ctx)
  {
...
static void ruby_strings_completion_wrapper_2(struct strings_completion_data *cb)
  {
  LOG_DEBUG(("--- invoking ruby callback, current data:\n\
  -----------------------\n\
  cb->dc              0x%08x\n\
  ...
  *cb->strings->data  0x%08x\n\
  =======================",
    cb->dc, cb->rc, cb->ctx, cb->strings,
    cb->strings->count, &cb->strings->count,
    cb->strings->data, &cb->strings->data, *cb->strings->data));
  cb->dc(cb->rc, cb->strings, cb->ctx);
  free(cb);
  }

static struct String_vector *string_vector_dup(struct String_vector *sv)
  {
  struct String_vector *sv_copy = malloc(sizeof(struct String_vector));

  memcpy(sv_copy,sv,sizeof(struct String_vector));
  return sv_copy;
  }

static void ruby_strings_completion_wrapper(strings_completion_t dc, int rc, struct String_vector *strings, void *ctx)
  {
  ...
  cb->dc = dc;
  cb->rc = rc;
  cb->strings = strings ? string_vector_dup(strings) : 0;
  cb->ctx = ctx ? strdup(ctx) : 0;

  LOG_DEBUG(("--- creating ruby thread."));
  rb_thread_create((void *)ruby_strings_completion_wrapper_2, cb);
  }
...
static void ruby_string_completion_wrapper_2(struct string_completion_data *cb)
  {
  LOG_DEBUG(("--- invoking ruby callback. current data:\n\
  -----------------------\n\
  cb->dc      0x%08x\n\
...
static void ruby_string_completion_wrapper(string_completion_t dc, int rc, char *path, void *ctx)
  {
  ...
  cb->dc = dc;
  cb->rc = rc;
  cb->path = path ? strdup(path) : 0;
  cb->ctx = ctx ? strdup(ctx) : 0;

  LOG_DEBUG(("--- creating ruby thread"));
  rb_thread_create((void *)ruby_string_completion_wrapper_2, (void *)cb);
  }

typedef struct acl_completion_data {
  acl_completion_t dc;
  int rc;
...
static void ruby_acl_completion_wrapper_2(struct acl_completion_data *cb)
  {
  LOG_DEBUG(("--- invoking ruby callback. current data:\n\
  -----------------------\n\
  cb->dc      0x%08x\n\
  ...
  cb->ctx     0x%08x\n\
  =======================",
    cb->dc, cb->rc, cb->acl, cb->stat, cb->ctx));
  cb->dc(cb->rc, cb->acl, cb->stat, cb->ctx);
  LOG_DEBUG(("--- callback returned, freeing struct."));
  free(cb);
  LOG_DEBUG(("--- struct freed."));
...
static struct ACL_vector *acl_vector_dup(struct ACL_vector *aclv)
  {
  struct ACL_vector *aclv_copy = malloc(sizeof(struct ACL_vector));

  memcpy(aclv_copy,aclv,sizeof(struct ACL_vector));
  return aclv_copy;
  }

static void ruby_acl_completion_wrapper(acl_completion_t dc, int rc, struct ACL_vector *acl, struct Stat *stat, void *ctx)
  {
  struct acl_completion_data *cb;

  cb = (acl_completion_data *)malloc(sizeof(acl_completion_data));
  cb->dc = dc;
  cb->rc = rc;
  cb->acl = acl ? acl_vector_dup(acl) : 0;
  cb->stat = stat ? stat_dup(stat) : 0;
  cb->ctx = ctx ? strdup(ctx) : 0;

  LOG_DEBUG(("--- creating ruby thread"));
  rb_thread_create((void *)ruby_acl_completion_wrapper_2, (void *)cb);
  }

typedef struct void_completion_data {
  void_completion_t dc;
  int rc;
  void *ctx;
  } void_completion_data;

static void ruby_void_completion_wrapper_2(struct void_completion_data *cb)
  {
  LOG_DEBUG(("--- invoking ruby callback. current data:\n\
  -----------------------\n\
  cb->dc      0x%08x\n\
...
static void ruby_void_completion_wrapper(void_completion_t dc, int rc, void *ctx)
  {
  struct void_completion_data *cb;

  cb = (void_completion_data *)malloc(sizeof(void_completion_data));
  cb->dc = dc;
  cb->rc = rc;
  cb->ctx = ctx ? strdup(ctx) : 0;

  LOG_DEBUG(("--- creating ruby thread"));
  rb_thread_create((void *)ruby_void_completion_wrapper_2, (void *)cb);
  }
#endif
        ...
        if (hdr.xid == WATCHER_EVENT_XID) {
            ...
            case COMPLETION_DATA:
                ...
                if (rc) {
                    ...
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_data_completion_wrapper(cptr->c.data_result,
                            rc, 0, 0, 0, (void *)cptr->data);
                    else
#endif
                    ...
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_data_completion_wrapper(cptr->c.data_result,
                            rc, res.data.buff, res.data.len, &res.stat,
                            (void *)cptr->data);
                    else
#endif
...
#ifdef HAVE_LIBRUBY
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_stat_completion_wrapper(cptr->c.stat_result,
                            rc, 0, (void *)cptr->data);
                    else
#endif
            ...
            case COMPLETION_STRINGLIST:
                ...
                if (rc) {
#ifdef HAVE_LIBRUBY
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_strings_completion_wrapper(cptr->c.strings_result,
                            rc, 0, (void *)cptr->data);
                    else
#endif
                    ...
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_strings_completion_wrapper(cptr->c.strings_result,
                            rc, &res.children, (void *)cptr->data);
                    else
#endif
                    cptr->c.strings_result(rc, &res.children, cptr->data);
                    // FIXME: deallocation wipes res before ruby reads it.
                    // For now, prefer leaking over failing..
...
#ifdef HAVE_LIBRUBY
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_string_completion_wrapper(cptr->c.string_result,
                            rc, 0, (void *)cptr->data);
                    else
#endif
            ...
            case COMPLETION_ACLLIST:
                ...
                if (rc) {
#ifdef HAVE_LIBRUBY
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_acl_completion_wrapper(cptr->c.acl_result,
                            rc, 0, 0, (void *)cptr->data);
                    else
#endif
                    ...
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_acl_completion_wrapper(cptr->c.acl_result,
                            rc, &res.acl, &res.stat, (void *)cptr->data);
                    else
#endif
...
#ifdef HAVE_LIBRUBY
                    if (getenv("RUBY_MRI_VERSION") != NULL)
                        ruby_void_completion_wrapper(cptr->c.void_result,
                            rc, (void *)cptr->data);
                    else
#endif
